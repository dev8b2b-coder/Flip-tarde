...
const navlistdata = [
  ...
  { id: 4, nav_name: "Platform", nav_link: "/platform" },
  {
    id: 5,
    nav_name: "Tools",
    nav_link: "/pip-calculator",
    dropdown: [
      { id: "pipcalculator", name: "Pip Calculator", link: "/pip-calculator" },
      { id: "economiccalendar", name: "Economic Calendar", link: "/economic-calendar" },
    ],
  },
    ...
    <div className={`pt-10 md:pt-12 pb-6 w-full ${isMenu ? "fixed" : "relative"}`}>
            ...
            {navlistdata.map((data) => (
              <div
                ...
                onMouseEnter={(e) => {
                  ...
                  if (data.nav_name === "Markets" || data.nav_name === "Tools") {
                    setOpenDropdown(data.nav_name);
                  } else {
          ...
          {navlistdata.map((data) => (
            <div key={data.id} className="relative group">
              {data.id === 2  ? (
                <div>
                  <p
                    className="relative z-10 flex items-center gap-2 text-secondary text-base font-medium py-2 px-3"
                  ...
                  <div
                    className={`${
                      isMarket ? "scale-[1] h-auto" : "scale-[0] h-0"
              ...
              <div>
                  ...
                  >
                    ...
                    <Image
                      ...
                      width={1000}
                      height={500}
                      className={`max-w-[18px] duration-500 transition-all ease-in-out ${isTools ? "rotate-180" : "rotate-0"}`}
                    />
                  </p>
                  <div
                    className={`${
                      isTools ? "scale-[1] h-auto" : "scale-[0] h-0"
                    } w-[250px] duration-500 transition-all ease-in-out rounded-2xl bg-primary`}
                  >
                    {navlistdata
                      .find((n) => n.nav_name === "Tools")
                      ?.dropdown.map((item) => (
                        <div key={item.id} className="text-start">
                          <Link
                            href={item.link}
      ...
      <div
        ...
        className={`
          ...
          ${openDropdown ? "scale-100 opacity-100 pointer-events-auto" : "scale-0 opacity-0 pointer-events-none"}
        `}
      >
            ...
            <div className="grid grid-cols-2 gap-4 py-16 ps-32">
              <div className="flex flex-col items-start">
                <div className="text-secondary text-5xl leading-16">
                  Start Your Trading <br /> Journey With <br /> Flip Trade
                </div>
                <div className="pt-8">
                  <Link href={"https://client.fliptradegroup.com/trader/registration"}>
                    <Button
                  ...
                  {navlistdata
                    ...
                    ?.dropdown?.map((item) => (
                      <div key={item.id} className="text-start">
                        <Link
                          href={item.link}
                          className="relative z-10 text-black hover:text-primary font_secondary hover:underline py-2 px-3 block rounded-lg text-base 2xl:text-xl pt-8 font-medium transition-all duration-500"
                        >
                          {item.name}
                        </Link>
                      </div>
